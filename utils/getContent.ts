//updated code after removal of downloadfromdropbox content
 ...
 * @returns {Promise<Content | null>} - The metadata JSON file as a JS object
 */
export async function getContent(): Promise<Content | null> {
  ...
  try {
    ...
    const content = await readFile(contentFilePath, { encoding: "utf-8" });
    const JSONContent = JSON.parse(content) as Content;
    logSuccess(`Successfully read content.json from ${contentFilePath}`);
    return JSONContent;
  } catch (error) {
    logError(`Could not read ${contentFilePath}: ${error}`);
    return null;
  }
}
